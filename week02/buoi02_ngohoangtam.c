...
// HỌC VIÊN BẮT ĐẦU VIẾT CODE TỪ ĐÂY
typedef void (*drivemodes)(void);
drivemodes drivemodes_table [3]={ drive_eco, drive_normal, drive_sport};
...
int main() {
    ...
    if (mode < sizeof(drivemodes_table) / sizeof(drivemodes_table[0])) {
        drivemodes_table[mode]();
    }else{
        printf("không có chế độ nào phù hợp\n");
    }
